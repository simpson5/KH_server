 ...
 * 
 * Servlet 생명주기
 * - was 구동내내 딱 하나의 객체만 만들어져서 처리된다. (singletone방식)
 * 1. Servlet객체생성(생성자 호출) - 최초 client 호출시 1회
 * 2. init메소드 호출 - 최초 client 호출시 1회
 * 3. HtppServlet의 service메소드 호출 - client 매요청마다
 * 4. 전송방시에 따라 doGet | doPost 호출 - client 매요청마다
 * 5. destroy호출(tomcat 종료시 객체 반환.) - 마지막 1회
 * 
 * 멱등
 * - 서비스 전후로 database의 상태가 바뀌지 않는 경우
 * - select -> GET
 * - insert update delete -> POST
 * - login 아이디와 PW가 url에 노출을 막기 위해 -> POST
 * 
 */
public class TestPerson1Servlet extends HttpServlet{
	...
	public TestPerson1Servlet() {
		super();
		System.out.println("기본 생성자 TestPerson1Servlet() 호출!");
	}
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) 
		...
		String[] foodArr = request.getParameterValues("food"); // String 배열을 리턴
		...
		out.println("<html>");
		out.println("<head>");
		out.println("<title>개치 검사 결과</title>");		
		out.println("</head>");
		out.println("<body>");
		...
		out.println("<p> 종아하는 중식은 " + Arrays.toString(foodArr) + "입니다.</p>");
		out.println("</body>");
		out.println("</html>");
	}
