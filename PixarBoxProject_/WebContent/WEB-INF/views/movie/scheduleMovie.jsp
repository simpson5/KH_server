...
$(()=> {
	...
	$.ajax({
		url: "<%=request.getContextPath() %>/movie/scheduleMovie/showScheduleMovie.do",
		...
		success: data=> {
			...
			for(let i in data) {
				...
				<%-- html += "<a href='<%=request.getContextPath() %>/movie/movieView/movieDescription.do?movieCode="+p.movieCode+"'><img src='<%=request.getContextPath() %>/images/"+p.movieImg+"' alt='"+p.movieTitle+"' class='poster'></a>";
				 --%> 
				 html += "<a href='"+"<%=request.getContextPath() %>/movie/movieView/movieDescription.do?movieCode="+p.movieCode+"&memberId=<%=memberLoggedIn!=null?memberLoggedIn.getMemberId():""%>'><img src='<%=request.getContextPath() %>/images/"+p.renamedFileName+"' class='poster'></a>";
